...
def cosine_annealing_lr(progress_remaining, initial_lr=1e-3, min_lr=1e-4, cycles=10, decay_factor=0.8):
    """
    Cosine annealing learning rate with warm-up and cooldown.
    """
    total_steps = 300000
    step = int((1 - progress_remaining) * total_steps)
    step_per_cycle = total_steps / cycles
    current_cycle = step // step_per_cycle
    cycle_progress = (step % step_per_cycle) / step_per_cycle
    amplitude = (initial_lr - min_lr) * (decay_factor ** current_cycle)

    # Warm-up for the first 1% of steps
    warmup_steps = total_steps * 0.01
    if step < warmup_steps:
        return 0 + (initial_lr - 0) * (step / warmup_steps)

    # Cooldown for the last 20% of steps
    cooldown_steps = total_steps * 0.2
    if step >= total_steps - cooldown_steps:
        return min_lr

    # Cosine decay in the middle
    return min_lr + 0.5 * amplitude * (1 + np.cos(np.pi * cycle_progress))

# -------------------------- Training Thread -------------------------

class TrainingThread(QtCore.QThread):
    """
    Qt-based training thread for policy model learning using SB3.
    """
    ...
    def run(self):
        ...
        feature_num_state = self.env.state_feature_length
        ...
        if algo == 'TD3':
            ...
            raise Exception('Invalid algorithm name: ', algo)
